...
export function useHorizontalSortable<T>(
    ...
    interface CtxProps {
        initialItemContentX: number
        initialFingerAbsoluteX: number
        initialLeftBound: number
    }
    ...
    const panGestureHandler = Gesture.Pan()
        .onStart((event: any) => {
            ...
            ctx.value = {
                ...
                ...{
                    ...
                    initialFingerAbsoluteX:
                    event.absoluteX,
                    initialLeftBound:
                    leftBound.value
                }
            }
            positionX.value = ctx.value.initialItemContentX;
            movingSV.value = true;
        ...
        .onUpdate((event) => {
            "worklet";
            const fingerDxScreen = event.absoluteX - ctx.value.initialFingerAbsoluteX;
            const scrollDeltaSinceStart = leftBound.value - ctx.value.initialLeftBound;
            positionX.value =
                ctx.value.initialItemContentX + fingerDxScreen + scrollDeltaSinceStart;
        })
        .onEnd((event: any) => {
            "worklet";
