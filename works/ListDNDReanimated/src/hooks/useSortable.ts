...
import {Gesture, PanGestureHandlerGestureEvent} from "react-native-gesture-handler";
...
export function useSortable<T>(
    ...
    useEffect(() => {
        ...
        const checkForHandle = (child: any): boolean => {
    ...
    interface CtxProps {
        initialItemContentY: number
        initialFingerAbsoluteY: number
        initialLowerBound: number
    }

    const ctx = useSharedValue<CtxProps>({
    ...
    const panGestureHandler = Gesture.Pan()
        .onBegin((event) => {
            ...
            console.log("0onBegin")

            ctx.value.initialItemContentY = positions.value[id] * itemHeight;
            ctx.value.initialFingerAbsoluteY = event.absoluteY;
            ctx.value.initialLowerBound = lowerBound.value;

            positionY.value = ctx.value.initialItemContentY;
            movingSV.value = true;
        ...
        .onUpdate((event) => {
            "worklet";
            const fingerDyScreen = event.absoluteY - ctx.value.initialFingerAbsoluteY;
            const scrollDeltaSinceStart = lowerBound.value - ctx.value.initialLowerBound;
            positionY.value =
                ctx.value.initialItemContentY + fingerDyScreen + scrollDeltaSinceStart;
        })
        .onEnd(() => {
            "worklet";
