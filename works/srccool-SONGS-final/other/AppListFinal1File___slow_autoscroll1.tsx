...
// optimised6: Velocity-based scroll speed constants
const MIN_SCROLL_FACTOR = 5.5;
const MAX_SCROLL_FACTOR = 3.0;
const VELOCITY_REF = 1.0; // reference velocity (pixels per ms) where factor = 1
...
export const useGesture = (
    ...
    scrollUp: (multiplier?: number) => void,   // optimised6: now accepts speed multiplier
    scrollDown: (multiplier?: number) => void, // optimised6: now accepts speed multiplier
    ...
    // optimised6: Shared value for current vertical velocity (from gesture)
    ...
    const onGestureUpdate = (newTop: number) => {
        ...
        // optimised6: Compute speed factor from current velocity
        const absVel = Math.abs(currentVelocity.value);
        const factor = Math.min(MAX_SCROLL_FACTOR, Math.max(MIN_SCROLL_FACTOR, absVel / VELOCITY_REF));
    ...
    const gestureController = Gesture.Pan()
        ...
        .onUpdate(e => {
            ...
            // optimised6: store the vertical velocity from gesture
            currentVelocity.value = Math.abs(e.velocityY); // in pixels per ms
...
const AnimatedList = (props:TListProps) => {
    ...
    // optimised6: scrollUp and scrollDown now accept a speed multiplier
    const scrollUp = (multiplier: number = 1) => {
        'worklet';
        const step = SCROLL_SPEED_OFFSET * multiplier;
        const newY =
            scrollY.value - step < 0
                ? 0
                : scrollY.value - step;
            ...
            <Animated.ScrollView
                ...
                {SONGS.map((eachSong,songNumber) => (
                    <ListItem
                        ...
                        scrollUp={scrollUp}       // optimised6: now accepts multiplier
                        scrollDown={scrollDown}   // optimised6: now accepts multiplier
                        scrollY={scrollY}
